...
let prosemirrorKeymap = require("prosemirror-keymap");
let prosemirrorHistory = require("prosemirror-history");
let prosemirrorCommands = require("prosemirror-commands");
let prosemirrorState = require("prosemirror-state");
let prosemirrorDropcursor = require("prosemirror-dropcursor");
let prosemirrorGapcursor = require("prosemirror-gapcursor");
//var prosemirrorMenu = require('prosemirror-menu');
let prosemirrorMenu = require("./menu");
let prosemirrorSchemaList = require("prosemirror-schema-list");
let prosemirrorInputrules = require("prosemirror-inputrules");
const { highlightPlugin } = require("prosemirror-highlightjs");
const hljs = require("highlight.js");
let { tableEditing, columnResizing, isInTable } = require("prosemirror-tables");
let { arrow, addColumnAfter, addColumnBefore, deleteColumn, addRowAfter, addRowBefore, deleteRow,
...
let { Fragment } = require("prosemirror-model");
...
let prefix = "ProseMirror-prompt";

function openPrompt(options) {
	let wrapper = document.body.appendChild(document.createElement("div"));
	wrapper.className = prefix;

	let mouseOutside = function (e) { if (!wrapper.contains(e.target)) { close(); } };
...
let Field = function Field(options) { this.options = options; };
...
let TextField = /*@__PURE__*/(function (Field) {
	function TextField() {
...
function buildMenuItems(schema) {
	...
	let cut = function (arr) { return arr.filter(function (x) { return x; }); };
...
function onshiftenter(state, dispatch) {
	if (isInTable(state)) {
		...
		arrow("vert", 1)(window.view.state, dispatch, window.view);
	}
	else {
...
function buildKeymap(schema, mapKeys) {
	...
	if (type = schema.nodes.hard_break) {
		let br = type, cmd = prosemirrorCommands.chainCommands(prosemirrorCommands.exitCode, function (state, dispatch) {
			dispatch(state.tr.replaceSelectionWith(br.create()).scrollIntoView());
			return true;
		});
...
function buildInputRules(schema) {
	let rules = prosemirrorInputrules.smartQuotes.concat(prosemirrorInputrules.ellipsis, prosemirrorInputrules.emDash), type;
