...
let rightClickedNotebookIndex;
// PAGE INDEX IS LOCAL TO THE NOTEBOOK
let rightClickedPageIndex;

let expandedNotebooks = [];
let activePage;

let draggedNotebookIndex;
let draggedPageIndex;
let draggedPagesNotebookIndex;
let draggingNotebook = false;
let draggingPage = false;

let fadeInSaveIndicator;
...
async function printPage(content, path, disableOpening = false) {
	...
	let data = await workerWindow.webContents.printToPDF({ pageSize: "A4", printBackground: true, scaleFactor: 100, marginsType: 0 });
