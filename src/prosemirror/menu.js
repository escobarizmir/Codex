...
let crel = _interopDefault(require("crelt"));
...
let prefix$1 = "ProseMirror-menu";

// ::- An icon or label that, when clicked, executes a command.
let MenuItem = function MenuItem(spec) {
	// :: MenuItemSpec
...
let Dropdown = function Dropdown(content, options) {
...
let DropdownSubmenu = function DropdownSubmenu(content, options) {
	this.options = options || {};
	this.content = Array.isArray(content) ? content : [content];
};
...
function renderGrouped(view, content) {
	...
	for (let i = 0; i < content.length; i++) {
		...
		for (let j = 0; j < items.length; j++) {
			let ref = items[j].render(view);
			let dom = ref.dom;
			let update$1 = ref.update;
...
let selectParentNodeItem = new MenuItem({
...
function blockTypeItem(nodeType, options) {
	let command = prosemirrorCommands.setBlockType(nodeType, options.attrs);
	let passedOptions = {
		run: command,
...
let prefix$2 = "ProseMirror-menubar";
...
const MenuBarView = function MenuBarView(editorView, options) {
	let this$1 = this;
	...
	let ref = renderGrouped(this.editorView, this.options.content);
	let dom = ref.dom;
	let update = ref.update;
	...
	if (options.floating && !isIOS()) {
		...
		this.scrollFunc = function (e) {
			let root = this$1.editorView.root;
