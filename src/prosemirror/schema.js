...
let prosemirrorModel = require("prosemirror-model");

let pDOM = ["p", 0], blockquoteDOM = ["blockquote", 0], hrDOM = ["hr"],
...
let nodes = {
	...
	image: {
		...
		toDOM: function toDOM(node) { let ref = node.attrs;
			let src = ref.src;
			let alt = ref.alt;
			let title = ref.title; return ["img", {src: src, alt: alt, title: title}]; }
	},
...
let emDOM = ["em", 0], strongDOM = ["strong", 0], codeDOM = ["code", 0], uDOM = ["u", 0];
...
let marks = {
	...
	link: {
		...
		toDOM: function toDOM(node) { let ref = node.attrs;
			let href = ref.href;
			let title = ref.title; return ["a", {href: href, title: title}, 0]; }
	},
...
let schema = new prosemirrorModel.Schema({nodes: nodes, marks: marks});
